...
int32_t MQTT_ReadLength(const uint8_t *stream, int32_t size, uint32_t *len)//2,4,0
{
	...
	int32_t i;
	const uint8_t *in = stream;//第2个字节存放剩余消息长度
	...
	*len = 0;
	for(i = 0; i < size; ++i)
	{
		*len += (in[i] & 0x7f) * multiplier;
		...
		if(!(in[i] & 0x80))//第2个字节第八位是0 如不果不是出现错误
		{
			return i + 1;
		}
		...
		{
			return -2;					// error, out of range 内存指针溢出
		}
	}
...
uint8_t MQTT_UnPacketRecv(uint8_t *dataPtr)
{
	...
	if(type == MQTT_PKT_PUBLISH)
	{
		...
		msgPtr = dataPtr + MQTT_ReadLength(dataPtr + 1, 4, &remain_len) + 1;  //跳过固定头具体看MQTT协议
		...
		if(strstr((const char *)msgPtr + 2, CMD_TOPIC_PREFIX) != NULL)	//如果是命令下发
			status = MQTT_PKT_CMD;
		else
...
printf( "Cmd1\r\n");
	...
	*cmdid = (char *)MQTT_MallocBuffer(37);				//cmdid固定36字节，多分配一个结束符的位置
	...
	memcpy(*cmdid, (const int8_t *)dataPtr, 36);			//复制cmdid
	dataPtr += 36;
		printf( "Cmd3\r\n");
	...
	*req = (char *)MQTT_MallocBuffer(*req_len + 1);		//分配命令长度+1
	...
	memcpy(*req, (const int8_t *)dataPtr, *req_len);		//复制命令
